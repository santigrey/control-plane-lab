...
ALLOWED_HOSTS = {
    ...
    "macmini":  "192.168.1.194",
    "cortez":   "100.70.77.115",
...
async def homelab_ssh_run(params: SSHRunInput) -> str:
    ...
    try:
        result = await asyncio.to_thread(ssh_run, params.host, params.command, params.timeout)
...
async def homelab_memory_search(params: MemorySearchInput) -> str:
    ...
    def _sync_impl():
        ...
        return rows
    try:
        rows = await asyncio.to_thread(_sync_impl)
...
async def homelab_memory_store(params: MemoryStoreInput) -> str:
    ...
    def _sync_impl():
        ...
        return row_id
    try:
        row_id = await asyncio.to_thread(_sync_impl)
...
async def homelab_file_read(params: FileReadInput) -> str:
    ...
    try:
        result = await asyncio.to_thread(ssh_run, params.host, f"cat {params.path}")
...
async def homelab_file_write(params: FileWriteInput) -> str:
    ...
    try:
        ...
        result = await asyncio.to_thread(ssh_run, params.host, remote_cmd, timeout=60)
...
async def homelab_agent_status() -> str:
    ...
    def _sync_impl():
        ...
        return status
    status = await asyncio.to_thread(_sync_impl)
...
async def homelab_create_task(params: CreateTaskInput) -> str:
    ...
    def _sync_impl():
        ...
        return row
    try:
        row = await asyncio.to_thread(_sync_impl)
