...
#top-badges { display: flex; gap: 6px; flex: 1; justify-content: flex-start; flex-wrap: wrap; }
...
#mute-btn { background: #1a1a2f; color: #8b949e; border: 1px solid #30363d; padding: 9px 12px; border-radius: 6px; font-size: 0.9rem; cursor: pointer; }
...
<div id="bottom-bar">
  ...
  <button id="mute-btn" onclick="toggleMute()" title="Mute/unmute voice">&#128264;</button>
  <button id="stop-voice-btn" onclick="stopVoice()" title="Stop speaking">&#9209;</button>
  <button id="private-btn" onclick="togglePrivate()" title="Private intimate mode (Goliath)">&#128275;</button>
</div>
<script>
let voiceMuted=false;let currentAudio=null;
let privateMode=localStorage.getItem('alexandra_private')==='1';
function togglePrivate(){privateMode=!privateMode;localStorage.setItem('alexandra_private',privateMode?'1':'0');updatePrivateUI();}
function updatePrivateUI(){var b=document.getElementById('private-btn');var i=document.getElementById('chat-input');if(!b||!i)return;if(privateMode){b.innerHTML='&#128274;';b.style.background='#f0883e';b.style.color='#000';i.style.borderColor='#f0883e';i.placeholder='Private mode - message Alexandra...';}else{b.innerHTML='&#128275;';b.style.background='';b.style.color='';i.style.borderColor='';i.placeholder='Message Alexandra...';}}
window.addEventListener('DOMContentLoaded',updatePrivateUI);
function toggleMute(){voiceMuted=!voiceMuted;const btn=document.getElementById("mute-btn");if(voiceMuted){btn.innerHTML="&#128263;";btn.classList.add("muted");btn.title="Voice muted";stopVoice();}else{btn.innerHTML="&#128264;";btn.classList.remove("muted");btn.title="Mute voice";}}
function stopVoice(){if(currentAudio){currentAudio.pause();currentAudio.currentTime=0;currentAudio=null;}document.getElementById("stop-voice-btn").style.display="none";}
...
async function sendChat(){const input=document.getElementById('chat-input');const msg=input.value.trim();if(!msg)return;input.value='';document.getElementById('send-btn').disabled=true;appendMsg('user',msg);setTyping(true);try{const r=await fetch(privateMode?'/chat/private?intimate=1':'/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:msg,session_id:SESSION_ID})});const d=await r.json();setTyping(false);const reply=d.response||JSON.stringify(d);appendMsg('alex',reply);setTimeout(loadRuns,1000);if(!voiceMuted){try{const sr=await fetch("/voice/speak",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:reply})});const ab=await sr.blob();const au=URL.createObjectURL(ab);currentAudio=new Audio(au);document.getElementById("stop-voice-btn").style.display="inline-block";currentAudio.onended=()=>{URL.revokeObjectURL(au);currentAudio=null;document.getElementById("stop-voice-btn").style.display="none";};currentAudio.play();}catch(ve){console.warn("voice speak error:",ve);}}}catch(e){setTyping(false);appendMsg('alex','Error: '+e.message);}finally{document.getElementById('send-btn').disabled=false;}}
...
try{const tr=await fetch('/voice/transcribe',{method:'POST',body:fd});const td=await tr.json();const text=(td.text||'').trim();if(!text){setVoiceStatus('');return;}setVoiceStatus('Sending to Alexandra...');appendMsg('user',text);setTyping(true);const cr=await fetch(privateMode?'/chat/private?intimate=1':'/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:text,session_id:SESSION_ID})});const cd=await cr.json();setTyping(false);const reply=cd.response||JSON.stringify(cd);appendMsg('alex',reply);if(!voiceMuted){setVoiceStatus('Alexandra is speaking...');try{const sr=await fetch('/voice/speak',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:reply})});const audioBlob=await sr.blob();const audioUrl=URL.createObjectURL(audioBlob);currentAudio=new Audio(audioUrl);document.getElementById('stop-voice-btn').style.display='inline-block';currentAudio.onended=()=>{setVoiceStatus('');URL.revokeObjectURL(audioUrl);currentAudio=null;document.getElementById('stop-voice-btn').style.display='none';};currentAudio.play();}catch(ve){setVoiceStatus('');console.warn('voice speak error:',ve);}}else{setVoiceStatus('');}}catch(e){setTyping(false);setVoiceStatus('Error: '+e.message);}};mediaRecorder.start();const btn2=document.getElementById('mic-btn');btn2.classList.add('recording');btn2.innerHTML='&#9209;';setVoiceStatus('<span class="rec-dot"></span>Recording… click to stop');}catch(e){setVoiceStatus('Mic error: '+e.message);}}
async function captureWebcam(){const btn=document.getElementById('cam-btn');const vs=document.getElementById('voice-status');try{btn.disabled=true;btn.textContent='⏳';if(vs){vs.textContent='Alexandra is looking...';vs.classList.add('visible');}const stream=await navigator.mediaDevices.getUserMedia({video:{width:{ideal:640},height:{ideal:480}}});const video=document.createElement('video');video.srcObject=stream;video.setAttribute('playsinline','true');await new Promise(r=>video.addEventListener('loadeddata',r,{once:true}));await video.play().catch(()=>{});await new Promise(r=>setTimeout(r,2000));const canvas=document.createElement('canvas');canvas.width=video.videoWidth||640;canvas.height=video.videoHeight||480;const ctx=canvas.getContext('2d');ctx.drawImage(video,0,0,canvas.width,canvas.height);stream.getTracks().forEach(t=>t.stop());const blob=await new Promise(r=>canvas.toBlob(r,'image/jpeg',0.9));const fd=new FormData();fd.append('file',blob,'webcam.jpg');const r=await fetch('/vision/analyze',{method:'POST',body:fd});const d=await r.json();appendMsg('alex',d.description||'Could not analyze image');setVoiceStatus('');}catch(e){appendMsg('alex','Camera error: '+e.message);setVoiceStatus('');}finally{btn.disabled=false;btn.textContent='📷';}}
let _audioCtx=null;
...
async function autoGreet(){
  ...
  try{
    ...
    for(let att=0;att<5;att++){
      ...
      const rows=[0,Math.floor(canvas.height/4),Math.floor(canvas.height/2),Math.floor(canvas.height*3/4)];let totalB=0;for(const row of rows){const px=ctx.getImageData(0,row,canvas.width,1).data;for(let i=0;i<px.length;i+=4){if(px[i]+px[i+1]+px[i+2]>50)totalB++;}}
      if(totalB>canvas.width*rows.length*0.3)break;
      await new Promise(r=>setTimeout(r,2000));
    }
    stream.getTracks().forEach(t=>t.stop());
    // Check if frame is still too dark after all retries
    const fRows=[0,Math.floor(canvas.height/4),Math.floor(canvas.height/2),Math.floor(canvas.height*3/4)];let finalBright=0;for(const row of fRows){const px=ctx.getImageData(0,row,canvas.width,1).data;for(let i=0;i<px.length;i+=4){if(px[i]+px[i+1]+px[i+2]>50)finalBright++;}}
    if(finalBright<canvas.width*fRows.length*0.3){appendMsg('alex','Camera is still warming up - try clicking the camera button in a moment.');if(vs){vs.textContent='';vs.classList.remove('visible');}_greeted=false;return;}
