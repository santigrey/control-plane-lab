...
async function loadStatus(){try{const r=await fetch('/healthz');const d=await r.json();const det=d.details||{};const badges=document.getElementById('top-badges');badges.innerHTML='';['api','postgres','ollama'].forEach(k=>{const val=det[k]||'unknown';const ok=val==='ok';const b=document.createElement('span');b.className='badge '+(ok?'ok':'err');b.textContent=k+': '+val;badges.appendChild(b);});const w=det.worker||{};const wb=document.createElement('span');wb.className='badge '+(w.status==='ok'||w.status==='idle'?'ok':'err');wb.textContent='worker: '+(w.status||'unknown');badges.appendChild(wb);}catch(e){document.getElementById('top-badges').innerHTML='<span class="badge err">status error</span>';}}
...
async function loadAgentTasks(){try{const r=await fetch('/dashboard/agent_tasks');const d=await r.json();const el=document.getElementById('agent-tasks');if(!d.tasks||!d.tasks.length){el.innerHTML='<div style="color:#8b949e">No tasks yet.</div>';return;}el.innerHTML='<table class="task-table"><thead><tr><th>Status</th><th>Title</th><th>Action</th></tr></thead><tbody>'+d.tasks.map(t=>{const sc=t.status==='pending_approval'?'status-pending':t.status==='approved'?'status-approved':t.status==='rejected'?'status-rejected':'';const act=t.status==='pending_approval'?`<button class="btn-approve" onclick="taskAction('${t.id}','approve')">&#10003;</button><button class="btn-reject" onclick="taskAction('${t.id}','reject')">&#10007;</button>`:'&mdash;';return`<tr><td class="${sc}">${t.status}</td><td>${esc(t.title||'')}</td><td>${act}</td></tr>`;}).join('')+'</tbody></table>';}catch(e){document.getElementById('agent-tasks').textContent='Error loading tasks';}}
...
async def dashboard_agent_tasks():
    try:
        with psycopg.connect(get_db_url()) as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT id, created_by, assigned_to, status, title, created_at
                    FROM agent_tasks ORDER BY created_at DESC LIMIT 20
                """)
                rows = cur.fetchall()
                return {"tasks": [{"id": str(r[0]), "created_by": r[1], "assigned_to": r[2],
