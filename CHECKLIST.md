...
**Last updated:** 2026-04-26 evening (Day 72) -- B2a closed; CAPACITY_v1.1 + CHARTERS_v0.1 + iCloud canon-flip ratified; B2b spec drafted
...
- [~] **B2b** -- logical replication CiscoKid -> Beast (WAL change + port rebind + pg_hba + UFW + schema bootstrap + publication + subscription + slot). DRAFTED 2026-04-26 in `tasks/B2b_logical_replication.md` (19341 bytes / ~440 lines). 12 acceptance gates, 4 structural picks flagged for CEO ratification (pg_hba bind-mount approach, downtime acceptance ~5-30s, subscription password handling, schema bootstrap transport). Depends on B2a complete (DONE) + Q4-Q7 picks (ratified). **Owner: Paco spec, PD execute. Awaiting CEO ratification.**
