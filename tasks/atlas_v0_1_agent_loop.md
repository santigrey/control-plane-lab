    ...
    while True:
        ...
        async with pool.acquire() as conn:
            row = await conn.fetchrow("""
                UPDATE atlas.tasks SET status='running', started_at=now()
                WHERE id = (
